...
EfficientNet, proposta da Tan e Le~\cite{tan2020efficientnetrethinkingmodelscaling}, nasce con l'obiettivo di massimizzare l'accuracy a parità di budget computazionale. A differenza degli approcci tradizionali, che scalano una sola dimensione della rete (ad esempio aumentando la risoluzione dell'input o semplicemente la profondità della rete), gli autori citati osservano che le tre dimensioni di scaling non sono indipendenti: bilanciarle tutte, contemporaneamente,  produce risultati migliori rispetto a scalarne una sola.
...
dove \(\alpha, \beta, \gamma\) sono costanti trovate tramite grid search su un modello base (\emph{EfficientNet-B0}, ottenuto tramite \emph{NAS}, Neural Architecture Search). Queste costanti stabiliscono come ripartire l'aumento di risorse tra le tre dimensioni; è invece il coefficiente $\phi$, scelto dall'utente, a determinare quante risorse complessive si vogliono impiegare. Ogni incremento di $\phi$ porta a un raddoppio approssimativo dei FLOPs della rete (\emph{floating point operations}, il numero di operazioni in virgola mobile richieste per un'inferenza), pari a $2^\phi$.
...
\begin{itemize}
    \item \textbf{ResNet}: introduce l'apprendimento residuo tramite le skip connection, mitigando il problema del vanishing gradient;
...
Una delle domande a cui questa tesi sperimentale tenta di rispondere è: come reagiscono queste diverse architetture a differenti tipologie di perdite controllate di informazioni nel training set?